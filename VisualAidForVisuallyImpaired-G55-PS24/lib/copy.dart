...
class _ImageUploadScreenState extends State<ImageUploadScreen> {
  ...
  Future<void> _uploadImage() async {
    ...
    var upload_user_image_req = http.MultipartRequest('POST', Uri.parse(upload_user_image));
    upload_user_image_req.files.add(await http.MultipartFile.fromPath('file', _image!.path));
    ...
    try {
      ...
      if (response.statusCode == 200) {
        String responseBody = await response.stream.bytesToString();
        upload_user_image_req.fields['caption'] = responseBody;
        var upload_user_image_res = await upload_user_image_req.send();
        if(upload_user_image_res.statusCode == 200){
            _showSnackbar("Image data sent to backend");
        }
